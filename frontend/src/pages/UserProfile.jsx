...
export default function UserProfile() {
  ...
  const fetchPosts = useCallback(async (userId, allPosts) => {
    ...
    try {
        const res = await fetch(`https://campusnet.onrender.com/post/author/${userId}${limitQuery}`, {
            method: 'GET',
  ...
  const handleConnect = async () => {
      ...
      try {
          const res = await fetch(`https://campusnet.onrender.com/user/connect/${profileUser.id}`, {
              method: 'POST',
              headers: { 'Authorization': `Bearer ${token}` },
          });
          const data = await res.json();
