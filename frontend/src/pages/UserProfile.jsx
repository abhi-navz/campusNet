...
import { useEffect, useState, useCallback, useRef } from "react"; // Added useRef temporarily to match template context, though not strictly needed here
// Added FiCheck and FiUsers for connection requests/acceptance UI
import {
  FiThumbsUp,
  FiMessageSquare,
  FiUsers,
...
export default function UserProfile() {
  ...
  const determineConnectionStatus = (profileData, visitorId) => {
    ...
    const isRequestSent =
      profileData.connectionRequests?.includes(visitorId) || false;

    // Note: isRequestReceived logic simplified/removed as it relies on fetching *my* requests,
    // which is better handled on the Network page. Here, we stick to the relationship visible
    // from the target profile.
    // const isRequestReceived = profileData.isOwnProfile && profileData.connectionRequests?.includes(profileData.id) || false;

    if (isConnected) return "Connected";
    if (isRequestSent) return "Pending";
    // if (isRequestReceived) return 'Request Received';
    return "Connect";
  };
    ...
    async (currentLoggedInUser) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setConnectionStatus(
            determineConnectionStatus(normalizedUser, currentLoggedInUser?.id)
          );
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setToastMessage({
        type: "error",
        message: "Network error. Could not send request.",
      });
  ...
  const handleAcceptRequest = async (senderId) => {
    ...
    try {
      ...
      setToastMessage({
        type: "error",
        message: "Network error during acceptance.",
      });
  ...
  const PendingRequestsSection = () => {
    ...
    if (
      ...
      !profileUser.connectionRequests ||
      profileUser.connectionRequests.length === 0
    ) {
      return null;
    }
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-6 p-4 bg-white rounded-xl shadow">
          <img
            src={
              ...
              "https://placehold.co/112x112/9253ed/ffffff?text=DU"
            }
          ...
          <div className="text-center sm:text-left flex-1 pt-3">
            ...
            <p className="text-gray-500 text-md mt-1">
              ...
              {profileUser.graduationYear && (
                <span>Grad {profileUser.graduationYear} | </span>
              )}
            ...
            <div className="flex justify-center sm:justify-start space-x-4 mt-3 text-sm">
              <span className="text-gray-700 font-semibold">
                {profileUser.connectionsCount || 0} Connections
              </span>
              <span className="text-gray-700 font-semibold">
          ...
          <div className="flex items-center sm:self-start mt-4 sm:mt-0 space-x-2">
            ...
            {connectionStatus === "Connected" && (
              <>
                <button className="bg-violet-700 text-white px-4 py-2 rounded-full font-medium hover:bg-violet-600 transition flex items-center">
